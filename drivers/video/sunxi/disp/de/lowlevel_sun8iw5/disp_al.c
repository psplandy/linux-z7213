...
s32 scaler_sync(u32 scaler_id)
{
	...
	spin_lock_irqsave(&al_data_lock, flags);
#endif
	memcpy(&scaler_info, &al_priv->fe_info.scaler_info, sizeof(disp_scaler_info));
	...
	spin_unlock_irqrestore(&al_data_lock, flags);
		...
		spin_unlock_irqrestore(&al_data_lock, flags);
#endif
		// We set the scaler params on sync, since some seem to not be buffered
		// despite what the doc says and what the original driver author(s) seemed
		// to believe. At least DEFE_BUF_ADDR0_REG is not buffered, and changes to
