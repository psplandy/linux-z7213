...
static int linein_to_spk_used		= 0;
static int linein_to_aif2_used		= 0;
...
//		codec_wr_control(0x48c, 0x7ff, 0, 0x0);
	...
	/*
	* 0x80: enable mclk and set mclk_div = 1;
	* 0x81: enable mclk and set mclk_div = 2;
	*/
...
static void aif2_clk_en(bool on)
{
	if (on) {
		...
		codec_wr_control(SUNXI_AIF2_CLK_CTRL, 0xf, AIF2_BCLK_DIV, 0x4);/*aif2/bclk=8:3.072M*/
		...
		codec_wr_control(SUNXI_AIF2_CLK_CTRL ,  0x3, AIF2_WORD_SIZ, 0x1);/*wss: 0x1:16; 0x3:24*/
		codec_wr_control(SUNXI_AIF2_CLK_CTRL ,  0x3, AIF2_DATA_FMT, 0x0);/*fmt:0:i2s, 1:L i2s*/
		codec_wr_control(SUNXI_AIF2_CLK_CTRL ,  0x1, AIF2_MONO_PCM, 0x0);/*stereo*/
	} else {
		if (linein_to_aif2_used&&(!linein_plugin)) {
			/*disable aif2,system clk from aif2*/
			codec_wr_control(SUNXI_SYSCLK_CTL, 0x1, AIF2CLK_ENA, 0x0);
			/*disable aif2 module*/
...
static void aif2_play_path_en(bool on)
{
	if (on) {
		...
		codec_wr_control(SUNXI_AIF2_MXR_SRC, 0x1, AIF2_ADCL_MXR_SRC_AIF1DA0L, 1);
		...
		if (linein_to_aif2_used&&(!linein_plugin)) {
			...
			codec_wr_control(SUNXI_AIF2_MXR_SRC, 0x1, AIF2_ADCL_MXR_SRC_AIF1DA0L, 0);
			codec_wr_control(SUNXI_AIF2_MXR_SRC, 0x1, AIF2_ADCR_MXR_SRC_AIF1DA0R, 0);
		}
	}
}

static void codec_spk_open(int on)
{
	int i = 0;
	int reg_val = 0;
...
static void linein_to_aif2_path_en(bool on)
{
	...
	if (on) {
		...
		codec_wr_control(SUNXI_AIF2_MXR_SRC, 0x1, AIF2_ADCL_MXR_SRC_ADCL, 1);
		...
		codec_wr_control(SUNXI_AIF2_MXR_SRC, 0x1, AIF2_ADCL_MXR_SRC_ADCL, 0);
...
static void linein_jack_events(unsigned long arg)
{
	...
	if (linein_to_spk_used||linein_to_aif2_used) {
		ret = gpio_get_value(linein_item.gpio.gpio);
		if (!ret) {
			if (linein_plugin == false) {
				linein_plugin = true;
				if (linein_to_spk_used) {
					linein_to_spk_path_en(linein_plugin);
				}
				if (linein_to_aif2_used) {
					linein_to_aif2_path_en(linein_plugin);
				#ifdef CONFIG_SND_SOC_APA3165
					apa3165_linein_play(linein_plugin);
				#endif
				}
			}
		} else {
...
//		codec_wr_control(0x48c, 0x7ff, 0, 0x0);
	...
	if (linein_to_spk_used||linein_to_aif2_used) {
		setup_timer(&linein_jack_timer, linein_jack_events, (unsigned long)0);
...
static int __init sndpcm_codec_probe(struct platform_device *pdev)
{
	...
	if (linein_to_spk_used||linein_to_aif2_used) {
		...
		type = script_get_item("audio0", "audio_linein_ctrl", &linein_item);
		if (SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
	...
	*/
	...
	if (linein_to_spk_used||linein_to_aif2_used) {
		...
		if (0 != req_status) {
			printk("request linein_item gpio failed!\n");
		}
